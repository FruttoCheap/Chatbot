...
def handle_svg_plot(method, question, connection, rag_db, input_chain, PRINT_SETTINGS):
    if method[0] == "NLP":
...
def handle_json_plot(method, question, rag_db, nlp_db, PRINT_SETTINGS):
    label = get_label_title(question, get_label_chain())
    
    if method[0] == "NLP":
        response = NLP(question, nlp_db, *initialize_nlp(URI_DB)[1:], PRINT_SETTINGS)
        chart_type = get_graph_type(get_type_chain(), question, PRINT_SETTINGS)
        labels = get_labels(get_labels_chain(), question, chart_type, response, PRINT_SETTINGS)
        data = get_data_NLP(labels, response, PRINT_SETTINGS)
    elif method[0] == "RAG":
        response = RAG(question, rag_db, stripOutput, PRINT_SETTINGS, is_for_plot=True)
        chart_type = get_graph_type(get_type_chain(), question, PRINT_SETTINGS)
        labels = get_labels(get_labels_chain(), question, chart_type, response, PRINT_SETTINGS)
        data = get_data_RAG(get_data_chain(), question, chart_type, labels, response, PRINT_SETTINGS)
...
def main():
    ...
    while True:
        ...
        if method == "REJECTED":
            ...
            handle_json_plot(method.split("|"), question, rag_db, nlp_db, PRINT_SETTINGS)
        elif PRINT_SETTINGS["call_SVG_plot"]:
            handle_svg_plot(method.split("|"), question, connection, rag_db, input_chain, PRINT_SETTINGS)
        else:
